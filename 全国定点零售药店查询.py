...
def fetch_data(regnCode):
    url = f'https://fuwu.nhsa.gov.cn/ebus/fuwu/api/nthl/api/CommQuery/queryRtalPhacBInfo'
    payload_template = {
        "data": {
...
def jsontoexecl():
   ...
   # 重命名列
    ...
    # 选择需要的列
    final_columns = ['省', '省编码', '市', '市编码', '统一社会信用代码', '药店名称', '机构编码', '经营范围', '详细地址', '联系电话',

                    '移动支付', '电子凭证', '电子处方', '电子票据', '医保钱包', '异地就医服务']
    df = df[final_columns]

    # 清理所有文本字段中的非法字符
    text_columns = ['省', '省编码', '市', '市编码', '统一社会信用代码', '药店名称', '机构编码', '经营范围', '详细地址', '联系电话']
